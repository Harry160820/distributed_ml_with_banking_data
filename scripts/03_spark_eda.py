...
Run: spark-submit scripts/03_spark_eda.py
"""
...
job_sub = df.groupBy("job", "y").count() \
             .orderBy("job")
...
ax.hist(age_pd["age"], bins=20, color="steelblue", edgecolor="white")
...
for label, grp in bal_pd.groupby("y"):
    ax.hist(grp["balance"].clip(-1000, 10000), bins=40,
            alpha=0.6, label=f"subscribed={label}")
...
# Summary stats -> CSV
df.describe(num_cols).toPandas().to_csv(
    PATHS["eda"] / "summary_stats.csv", index=False
)
